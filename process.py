...
def send_token(next_host, next_port, token):
    ...
    sock.sendto(message.encode(), (MULTICAST_GROUP_ROUND_TIMES, MULTICAST_PORT_ROUND_TIMES))
...
def listen_multicast():
    ...
    mreq = struct.pack("4sl", socket.inet_aton(MULTICAST_GROUP_FIREWORKS), socket.INADDR_ANY)
...
def main(args):
    print(f"[Process {args.id}] Starting with initial probability {args.initial_p} and k = {args.k}")
    try:
        listen_multicast()
        ...
        while True:
            ...
            if token.get('silent_rounds') and token['silent_rounds'] >= args.k:
                print(f"[Process {args.id}] Received token with silent rounds >= k, terminating.")
